...
public class PessoaService implements PessoaBuscaPorNome {
	...
	public Pessoa update(Pessoa entity) {
		...
		try {
			...
			em.getTransaction().begin();
			em.merge(entity);
			// usando o provider do hibernate ao invés da jpa (vantagem: não faz select |
			// desvantagem: se não houver atualização, vai atualizar assim mesmo; se mudar
			// para outro provider, a aplicação quebrará em tempo de execução sem dizer
			// previamente onde errou)
	...
	public List<Pessoa> searchByName(String name) {
		...
		try {
			 ...
			 * .createQuery("from Pessoa p where lower(p.nome) like lower(concat('%',:nome,'%'))"
			 * , Pessoa.class) .setParameter("nome", name).getResultList();
			 */
